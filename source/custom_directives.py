    ...
    def run(self):
        ...
        try:
            ...
            if 'figure' in self.options:
                ...
                save_figname = os.path.join('source/_static/thumbs/',
                                            os.path.basename(figname))

                try:
                    os.makedirs('source/_static/thumbs')
                except OSError:
                    pass
                ...
                img_name = save_figname.replace('source/', '')
                thumbnail_rst = re.sub(r'..\sfigure::\s.*\.png',
                                       '.. figure:: /{}'.format(img_name),
                                       thumbnail_rst)
    ...
    def run(self):
        try:
            ...
            if 'figure' in self.options:
                env = self.state.document.settings.env
                rel_figname, figname = env.relfn2path(self.options['figure'])
                thumbnail = os.path.join('source/_static/thumbs/', os.path.basename(figname))
